...
public class RepositorieTransactions : IRepositorieTransactions
{
    ...
    public async Task<IEnumerable<Transactions>> GetTransactions(int AccountId)
    {
        ...
        return await connection.QueryAsync<Transactions>(@"SELECT t.Id, t.UserId, t.TransactionDate, t.Total, [o].Description as OperationTypeDescription, t.Description, a.Name as AccountName, c.Name as 'CategoryName' 
            FROM Transactions as t 
            JOIN Categories as c ON t.CategoryId=c.Id
            JOIN Accounts as a ON t.AccountId=a.Id 
            JOIN OperationTypes AS [o] ON  [o].Id=c.OperationTypeId
